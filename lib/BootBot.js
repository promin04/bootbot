...
class BootBot extends EventEmitter {
  ...
  getUserPicture(userId) {
    const url = `https://graph.facebook.com/v2.6/${userId}/picture?access_token=${this.accessToken}`;
    return fetch(url)
      .catch(err => console.log(`Error getting user picture: ${err}`));
  }
  ...
  comment(id, message) {
    ...
    return fetch(`https://graph.facebook.com/v2.6/${id}/comments?access_token=${this.accessToken}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      .then(res => {
        if (res.error) {
          console.log('Comment Error received. For more information about error codes, see: https://goo.gl/d76uvB');
          console.log(res.error);
        }
        return res;
      })
  ...
  _handleEvent(type, event, data) {
    var recipient = null;
    if(type === 'authentication' && !event.sender) {
      recipient = { user_ref: event.optin.user_ref };
    } else {
  ...
  _handleEventFeed(type, event, data) {
    const chat = new Chat(this, event.from.id);
    this.emit(type, event, chat, data);
  }
    ...
    data.entry.forEach((entry) => {
      ...
      if(entry.messaging) {
        ...
        entry.changes.forEach((event) => {
          // not self
          event.value.owner = false;
          if(event.value && event.value.from && event.value.from.id && entry.id === event.value.from.id) {
            event.value.owner = true;
          }

          this._handleEventFeed('feed', event.value);
        });
      }
    });
  }
